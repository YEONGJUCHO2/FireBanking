...
- Holding quantity edit/delete after initial registration.
- Other investment manual amount.
- US-listed holdings manual-assist valuation: ticker/name, quantity, USD price, and
  manual USD/KRW rate are multiplied into KRW. This is not market-data automation.
...
- Liability balance, interest, principal edit/delete after initial registration.
...
CSV/bulk import remains deferred intentionally. The first release is optimized for
households with roughly 10 or fewer tracked investment rows. If alpha recruiting targets
heavy investors with 10-20+ holdings, bulk import must become a separate follow-up
...
- Holding save action
- Holding quantity edit/delete action
- Liability save action
- Liability balance/interest/principal edit/delete action
- US-listed manual-assist valuation input
...
If an asset snapshot exists but the matching cashflow snapshot is missing, the dashboard
must not show a full FIRE result for that month. It may show an asset-only historical row
with a clear "생활비 스냅샷 없음" label, or hide that month from the FIRE history list.
Do not mix asset data from one month with cashflow data from another month.

## R0 To R1 Migration Policy

Existing R0 users have one lump-sum `investableNetWorth`. R1 splits that value into:

```text
cash + domestic auto-valued holdings + other investments + US-listed manual-assist holdings
```

On first R1 asset setup:

1. Load the user's latest R0 `investableNetWorth`.
2. Let the user register domestic holdings and optional US-listed manual-assist holdings.
3. Compute the registered investment total.
4. If `registered total < latest R0 investableNetWorth`, prefill the difference into
   `otherInvestmentAmount` and label it "이전 입력에서 가져온 기타 투자자산".
5. If `registered total > latest R0 investableNetWorth`, do not create a negative
   other investment. Show an explanation that the new detailed inputs have replaced
   the old lump-sum estimate.

The first R1 experience must explain why "FIRE 계산 순자산" may differ from the old R0
number:

```text
이전에는 투자가능 순자산을 한 번에 입력했어요.
이제는 자동평가 자산과 기타 투자자산으로 나누어 계산해요.
```
...
src/features/assets/actions/updateHolding.ts
src/features/assets/actions/updateHolding.test.ts
src/features/assets/actions/deleteHolding.ts
src/features/assets/actions/deleteHolding.test.ts
src/features/assets/actions/saveLiability.ts
src/features/assets/actions/saveLiability.test.ts
src/features/assets/actions/updateLiability.ts
src/features/assets/actions/updateLiability.test.ts
src/features/assets/actions/deleteLiability.ts
src/features/assets/actions/deleteLiability.test.ts
...
If Kiwoom credentials are missing, the UI should disable domestic auto-search rather
than letting the user type into a broken search box. The action still returns a
recoverable Korean error for direct calls:

```text
종목 검색을 준비 중이에요. 잠시 후 다시 시도해주세요.
```

The panel must still offer `미국상장 수동 계산` and `기타 투자자산` entry while domestic
...
- Create: `src/features/assets/actions/updateHolding.ts`
- Create: `src/features/assets/actions/updateHolding.test.ts`
- Create: `src/features/assets/actions/deleteHolding.ts`
- Create: `src/features/assets/actions/deleteHolding.test.ts`
- Create: `src/features/assets/actions/saveLiability.ts`
- Create: `src/features/assets/actions/saveLiability.test.ts`
- Create: `src/features/assets/actions/updateLiability.ts`
- Create: `src/features/assets/actions/updateLiability.test.ts`
- Create: `src/features/assets/actions/deleteLiability.ts`
- Create: `src/features/assets/actions/deleteLiability.test.ts`

- [ ] **Step 1: Write action validation tests**

Create tests that verify create/update/delete validation:
...
```ts
it("updates holding quantity for an admin couple member");
it("deletes a holding for an admin couple member");
it("updates liability balance, monthly interest, and monthly principal");
it("deletes a liability for an admin couple member");
it("rejects updates and deletes from non-admin members");
```
...
- [ ] **Step 3: Implement authenticated create/update/delete actions**
...
`deleteHolding` deletes the selected holding.

`updateLiability` updates:
...
All create/update/delete actions must require admin membership and must scope mutations
by both `id` and `couple_id` so one couple cannot update another couple's asset rows.
...
```bash
npm run test -- src/features/assets/actions/saveHolding.test.ts src/features/assets/actions/updateHolding.test.ts src/features/assets/actions/deleteHolding.test.ts src/features/assets/actions/saveLiability.test.ts src/features/assets/actions/updateLiability.test.ts src/features/assets/actions/deleteLiability.test.ts
```
...
```bash
git add src/features/assets/actions/saveHolding.ts src/features/assets/actions/saveHolding.test.ts src/features/assets/actions/updateHolding.ts src/features/assets/actions/updateHolding.test.ts src/features/assets/actions/deleteHolding.ts src/features/assets/actions/deleteHolding.test.ts src/features/assets/actions/saveLiability.ts src/features/assets/actions/saveLiability.test.ts src/features/assets/actions/updateLiability.ts src/features/assets/actions/updateLiability.test.ts src/features/assets/actions/deleteLiability.ts src/features/assets/actions/deleteLiability.test.ts
...
US-listed manual-assist fields:
...
Do not call this "자동평가". Label it "미국상장 수동 계산" so users do not confuse it
with provider-backed domestic auto valuation.
...
은행 앱의 대출 상환 안내에서 이번 달 이자와 원금 항목을 확인할 수 있어요.
```

Liability purpose guide:

```text
거주 부동산 관련: 주담대, 전세자금대출처럼 거주 자산과 묶인 빚
투자 관련: 주식담보대출, 투자 목적 신용대출처럼 투자자산을 사기 위한 빚
생활/신용 관련: 카드론, 생활비 신용대출, 자동차 할부처럼 생활 지출과 묶인 빚
기타: 위 셋 중 확신이 없을 때
```

Month-end timing copy:

```text
월말 스냅샷은 매월 말 자동으로 만들어져요.
월말 실행 이후 등록한 자산은 다음 월부터 반영돼요.
```
...
If an asset snapshot exists but the matching `monthly_cashflow_snapshots` row is missing,
do not render a full FIRE dashboard for that month. Show an asset-only row with
`생활비 스냅샷 없음` or hide that month from the FIRE history picker.
...
- Domestic listed holdings: Tasks 2, 4, 5, 6, 7.
- Search plus quantity input: Tasks 5, 6, 7.
- Holding quantity edit/delete: Tasks 6, 7.
- US-listed manual-assist valuation: Tasks 1, 7, 11.
- Pension/IRP account category: Tasks 2, 6, 7, 11.
- Liabilities, edit/delete, and debt cashflow: Tasks 1, 2, 6, 7, 11.
