...
function ConnectedPartnerCard({ coupleId }: { coupleId: string }) {
  return (
    <section className="rounded-card border border-fb-green/20 bg-fb-green-50 p-5 shadow-card">
      <p className="text-sm font-bold text-fb-green">배우자 연결됨</p>
      ...
      <form action={unlinkPartnerFormAction} className="mt-4">
        ...
        <button
          type="submit"
          className="fb-focus rounded-button border border-fb-green/30 bg-white px-4 py-2 text-sm font-bold text-fb-green hover:bg-fb-green-100"
        >
          연동 해제
        </button>
      </form>
