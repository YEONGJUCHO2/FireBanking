    ...
    <div className="group" style={{ width }}>
      {label ? (
        <div className="mb-3 flex items-center gap-2">
          <span className="flex size-6 items-center justify-center rounded-full bg-fb-green text-xs font-bold text-white shadow-card">{number}</span>
          <div>
      ...
      <div className="relative transition duration-700 ease-out group-hover:-translate-y-2" style={{ width, height }}>
        <div className="fb-phone-frame h-[760px] w-[320px] origin-top-left overflow-hidden bg-fb-surface transition duration-700 ease-out group-hover:shadow-elevated" style={{ transform: `scale(${scale})` }}>
          {children}
        </div>
      </div>
    </div>
...
export function LoginScreenPreview() {
  return (
    <div className="relative flex h-full flex-col overflow-hidden px-5 py-6">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(255,253,248,0.72)_0%,rgba(255,253,248,0.34)_48%,rgba(18,61,51,0.34)_100%)]" />
      ...
      <div className="relative z-10 mt-auto">
        ...
        <div className="mt-5 flex gap-3 rounded-card border border-white/75 bg-white/84 p-4 text-xs leading-5 text-fb-muted shadow-card backdrop-blur">
          <Icon name="shield" className="mt-0.5 size-4 shrink-0 text-fb-green" />
          <p><span className="font-bold text-fb-green">안전하게 보호돼요.</span> 결과 계산에 필요한 정보만 다룹니다.</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex h-full flex-col px-5 pb-5 pt-3">
      ...
      <section className="mt-5">
        ...
        <p className="mt-4 text-[13px] font-semibold leading-[1.75] tracking-normal text-fb-muted">
          정확하지 않아도 괜찮아요.
          <br />
          지금은 첫 거리감을 보는 단계예요.
        </p>
      ...
      <form className="mt-4 flex min-h-0 flex-1 flex-col">
        <p className="mb-2 text-right text-[11px] font-bold text-fb-muted">단위: 만원</p>
    ...
    <label className="grid grid-cols-[132px_1fr] items-center gap-2">
      ...
      <span className="relative block">
        <input
          ...
          aria-label={label}
        />
        <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] font-bold text-fb-muted">만원</span>
      </span>
    </label>
  )
}
    ...
    <div className="flex h-full flex-col px-4 pb-0 pt-3">
      ...
      <header className="mt-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[10px] font-medium leading-4 text-fb-muted">우리의 경제적 자유 현황을 한눈에</p>
        </div>
        <div className="flex gap-2">
          <span className="grid size-8 place-items-center rounded-full border border-fb-line bg-white shadow-card"><Icon name="bell" className="size-4 text-fb-muted" /></span>
          <span className="grid size-8 place-items-center rounded-full border border-fb-line bg-white shadow-card"><Icon name="users" className="size-4 text-fb-muted" /></span>
        </div>
      </header>
      ...
      <div className="mt-4 space-y-3">
        <section className="relative min-h-[156px] overflow-hidden rounded-[0.9rem] p-4 text-white shadow-soft">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(18,61,51,0.88),rgba(18,61,51,0.28))]" />
          <div className="relative z-10">
            <div className="flex justify-between gap-3">
              <p className="text-[10px] font-bold opacity-90">예상 FIRE 도달 시점</p>
              <span className="rounded-full bg-white/90 px-2 py-1 text-[10px] font-bold text-fb-green">정상</span>
            </div>
        ...
        <section className="rounded-[0.8rem] border border-fb-line bg-fb-sand/60 p-3 text-[10px] leading-4 text-fb-muted">
          이 결과는 참고용 시뮬레이션이에요.
        </section>
      </div>
...
function MockMetric({ title, value, delta, caption, small = false, wide = false }: { title: string; value: string; delta?: string; caption?: string; small?: boolean; wide?: boolean }) {
  return (
    <section className={wide ? 'rounded-[0.8rem] border border-fb-line bg-white p-3 shadow-card' : 'rounded-[0.8rem] border border-fb-line bg-white p-3 shadow-card'}>
      <p className="text-[10px] font-bold tracking-normal text-fb-muted">{title}</p>
      ...
      {delta ? <p className="mt-1.5 text-[10px] font-bold text-fb-green">전월 대비 {delta}</p> : null}
      {caption ? <p className="mt-1.5 text-[10px] font-bold leading-4 text-fb-muted">{caption}</p> : null}
    </section>
  )
}
...
function MockBottomNav({ active }: { active: string }) {
  ...
  return (
    <nav className="mt-auto border-t border-fb-line bg-fb-surface/95 px-1 py-2">
      <div className="grid grid-cols-5">
        {items.map((item) => (
          <span key={item.label} className={item.label === active ? 'flex flex-col items-center gap-1 text-[9px] font-bold text-fb-green' : 'flex flex-col items-center gap-1 text-[9px] font-bold text-fb-muted'}>
...
export function InviteScreenPreview() {
  return (
    <div className="flex h-full flex-col px-4 pb-4 pt-3">
      ...
      <div className="mt-7 space-y-4 text-[11px] font-bold leading-5 text-fb-muted">
      ...
      <div className="mt-auto rounded-[0.95rem] border border-fb-line bg-fb-green-50/55 p-3 shadow-card">
        <p className="pb-3 text-center text-[11px] font-bold leading-5 text-fb-muted">
          언제든 참여할 수 있어요.
          <br />
          부담 없이 초대해 보세요.
        </p>
        <div className="grid gap-2">
          <button className="h-11 rounded-[0.55rem] bg-fb-green text-[13px] font-bold text-white shadow-card">초대 링크 생성</button>
  ...
  return (
    <div className="flex h-full flex-col px-5 pb-5 pt-3">
      ...
      <header className="mt-2">
        ...
        <p className="mt-3 text-[12px] font-medium leading-5 text-fb-muted">정확하지 않아도 괜찮아요.<br />지금은 첫 거리감을 보는 단계예요.</p>
      </header>
      <p className="mt-5 text-right text-[10px] font-bold text-fb-muted">단위: 만원</p>
      <div className="mt-3 space-y-5">
        {liteRows.map((row) => (
          <CompactMoneyRow key={row.label} label={row.label} value={row.value} />
        ))}
      </div>
      <div className="mt-9 rounded-[0.75rem] bg-fb-green-50 p-4 text-center text-[12px] font-bold text-fb-green">
        지난달과 같아요
      </div>
      <p className="mt-5 rounded-[0.75rem] bg-fb-sand/60 p-4 text-[11px] leading-5 text-fb-muted">지난달과 같다면 빠르게 진행할 수 있어요.</p>
      <button className="mt-auto h-12 rounded-[0.65rem] bg-fb-green text-[13px] font-bold text-white shadow-card">다음</button>
      <div className="pt-4">
        <ProgressStepper steps={['입력', '확인', '완료']} current={0} />
      </div>
    </div>
  ...
  return (
    <div className="flex h-full flex-col px-4 pb-5 pt-3">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 rounded-[0.9rem] border border-fb-line bg-white p-3 shadow-card">
        <button className="h-9 rounded-[0.5rem] bg-fb-green text-[12px] font-bold text-white">설정</button>
        <button className="h-9 rounded-[0.5rem] bg-fb-sand/70 text-[12px] font-bold text-fb-muted">결과 비교</button>
      </div>
      <section className="mt-4 space-y-2.5">
        {[
          ...
          <div key={label} className="flex items-center justify-between border-b border-fb-line/70 py-1.5">
            ...
            <span className="text-[11px] font-bold text-fb-muted">{value}</span>
          </div>
        ))}
      </section>
      <section className="mt-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="text-[10px] font-bold text-fb-muted">현재 합계 230만원</span>
        </div>
        <div className="space-y-1.5">
          {['주거비(대출/관리비)', '통신비', '보험료', '구독/멤버십', '차량유지비', '교육비', '기타 고정비'].map((label, index) => (
            <div key={label} className="grid grid-cols-[18px_1fr_58px] items-center gap-2 rounded-[0.45rem] bg-white px-2 py-1.5 shadow-[0_0_0_1px_var(--color-fb-line)]">
              <span className="grid size-4 place-items-center rounded-[0.18rem] bg-fb-green text-white"><Icon name="check" className="size-3" /></span>
      ...
      <section className="mt-4 rounded-[0.8rem] border border-fb-line bg-white p-3 shadow-card">
        ...
        <div className="mt-4 flex h-28 items-end justify-center gap-10 rounded-[0.7rem] bg-fb-green-50 p-3">
          <div className="flex flex-col items-center gap-1">
            <span className="text-[10px] font-black text-fb-ink">4.2억</span>
            <span className="h-12 w-9 rounded-t bg-fb-slate/35" />
          </div>
          <div className="flex flex-col items-center gap-1">
            <span className="text-[10px] font-black text-fb-green">5.6억</span>
            <span className="h-20 w-9 rounded-t bg-fb-green" />
          </div>
        </div>
      </section>
    </div>
