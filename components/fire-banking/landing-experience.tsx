...
/**
 * Calm entry screen content — matches /showcase LoginScreenPreview spec.
 * Same markup is rendered both at `/` (real, via LandingExperience) and inside
 * the /showcase phone frame so the two surfaces stay pixel-aligned.
 *
...
export function LoginScreenContent({ authError }: { authError?: string | null } = {}) {
  return (
    <section
      ...
      className="flex flex-1 flex-col bg-fb-page px-5 pb-6 pt-14"
    >
      ...
      <div data-od-id="product-definition" className="mt-16">
        ...
        <h2 className="mt-3.5 text-[12px] font-medium leading-[1.55] text-fb-ink-2">
      ...
      {authError ? (
        <div
          ...
          role="alert"
          className="mb-3 rounded-[12px] border border-fb-cautionary-soft bg-fb-cautionary-soft px-4 py-3 text-[12px] font-medium leading-[1.5] text-fb-cautionary-ink"
        >
