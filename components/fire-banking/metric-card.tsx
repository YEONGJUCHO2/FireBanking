  ...
  return (
    <section className={cn('fb-card', sizeClass[size].card, className)}>
      <div className="flex items-start justify-between gap-2">
        <p className="text-xs font-bold tracking-normal text-fb-muted">{title}</p>
        {statusLabel ? <StatusPill label={statusLabel} status={statusMap[variant]} /> : null}
      </div>
      <p className={cn('mt-2 font-bold tracking-normal text-fb-ink', sizeClass[size].value)}>{value}</p>
