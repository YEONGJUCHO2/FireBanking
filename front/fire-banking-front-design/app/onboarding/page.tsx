    ...
    <MobileAppShell>
      ...
      <form className="space-y-5 px-5 pb-6">
        <div className="flex items-center justify-between rounded-card border border-fb-line bg-fb-green-50 px-4 py-3 text-sm text-fb-muted">
          <span>입력 단위</span>
          <strong className="text-fb-green">만원</strong>
        </div>

        <div className="space-y-4">
          {onboardingRows.map((row) => <MoneyInputRow key={row.label} label={row.label} value={row.value} helper={row.helper} />)}
        </div>

        <div className="flex gap-3 rounded-card border border-fb-line bg-fb-surface p-4 text-sm leading-6 text-fb-muted">
          <Icon name="info" className="mt-0.5 size-5 shrink-0 text-fb-green" />
