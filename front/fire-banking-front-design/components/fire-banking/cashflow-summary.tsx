  ...
  return (
    <section className="fb-card p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className="text-base font-bold tracking-[-0.03em] text-fb-ink">월 현금흐름 요약</h2>
      ...
      <div className={cn('grid gap-2', compact ? 'grid-cols-4' : 'grid-cols-2 md:grid-cols-4')}>
        {steps.map((step, index) => (
          <div key={step.label} className="rounded-soft border border-fb-line bg-fb-surface p-3">
            ...
            <p className={cn('mt-1 text-lg font-bold tracking-[-0.04em]', step.value < 0 ? 'text-fb-danger' : 'text-fb-ink')}>{formatManWon(step.value)}</p>
          </div>
        ))}
      </div>
      {remainingMan < 0 ? (
